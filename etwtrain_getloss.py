...
def train():
    ...
    loss_et = 1 - dice_coe_et + mean_var(prob_et[...,0], gt_et)*0.1
    loss_et_dice = 1 - dice_coe_et  #---
    loss_et_test = mean_var(prob_et[...,0], gt_et)  #---
    ...
    loss_tc = 1 - dice_coe_tc + mean_var(prob_tc[...,0], gt_tc)*0.1
    loss_tc_dice = 1 - dice_coe_tc  #---
    loss_tc_test = mean_var(prob_tc[...,0], gt_tc)  #---
    ...
    loss_wt = 1 - dice_coe_wt + mean_var(prob_wt, gt_wt)*0.1
    loss_wt_dice = 1 - dice_coe_wt  #---
    loss_wt_test = mean_var(prob_wt, gt_wt)  #---
    ...
    with tf.Session(config = config) as sess:
        file =  open('info_etw_train_getloss.txt', 'a')
        sess.run(init)
        if fine_tune:
            ckpt_file = tf.train.latest_checkpoint('./saver/etw0')
            # saver.restore(sess, ckpt_file)
            saver.restore(sess, './saver/etw/Anet.ckpt-3')
        ...
        for epoch in range(epochs):
            ...
            for begin in range(0, len(Load.sample_names), 48):
                ...
                for sampling_time in range(sampling_times):
                    for img, label in loader.get_batch_of_patch(batch_size, p_shape):
                        ...
                        (loss_et_dice_np, loss_et_test_np, loss_tc_dice_np, loss_tc_test_np, 
                                loss_wt_dice_np, loss_wt_test_np) = sess.run(
                                [loss_et_dice, loss_et_test, loss_tc_dice, loss_tc_test, 
                                loss_wt_dice, loss_wt_test], feed_dict={x:img, y:label})
                        loss_ls.append([loss_et_dice_np, loss_et_test_np, loss_tc_dice_np, 
                                loss_tc_test_np, loss_wt_dice_np, loss_wt_test_np])
                        steps += 1

                        loss_str = (f"{steps:<4} et_dice:{loss_et_dice_np:.4f} et_test:{loss_et_test_np:.4f} "
                                    f"tc_dice:{loss_tc_dice_np:.4f} tc_test:{loss_tc_test_np:.4f} "
                                    f"wt_dice:{loss_wt_dice_np:.4f} wt_test:{loss_wt_test_np:.4f}")
                        print(loss_str)
                        print(loss_str, file=file, flush=True)
            #             if steps%20 == 0:
            ...
            # saver.save(sess, w_path, global_step = epoch)
        mean_loss = np.mean(loss_ls, axis=0)
        print(mean_loss)
        file.close()
